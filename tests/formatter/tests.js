...
function subgroup(el, iterations, params) {
  ...
  const stave1 = f
    .Stave({ x: 15, y: 40, width: options.systemWidth })
    .setClef("treble");
  const notes1 = [
  ...
  const stave2 = f
    .Stave({ x: 15, y: 160, width: options.systemWidth })
    .setClef("bass");
  ...
  addSubGroup(notes3[1], [
    f.ClefNote({ type: "treble", options: { size: "small" } }),
  ]);
  addSubGroup(notes3[5], [
    f.ClefNote({ type: "bass", options: { size: "small" } }),
  ]);
  ...
  f.draw();
  if (options.debug) {
    VF.Formatter.plotDebugging(
      f.getContext(),
      formatter,
      stave1.getNoteStartX(),
      20,
      320,
    );
  }
...
function tuplets(el, iterations, params) {
  ...
  const voices = [
    ...
    score.tuplet(score.notes("a4/32, a4, a4, a4, a4, a4, a4"), {
      notesOccupied: 8,
    }),
  ];
...
function multistave(el, iterations, params) {
  ...
  var notes12 = score.notes(
    'ab4/4, bb4/8, (cb5 eb5)/4[stem="down"], d5/8[stem="down"]',
  );
  ...
  var notes22 = score.notes("(eb4 ab4)/4., (c4 eb4 ab4)/4, db5/8", {
    stem: "up",
  });
