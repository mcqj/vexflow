...
Options:
  --backends=(all|jsdom|playwright) : Specify which backend(s) to run.
...
const appMain = async () => {
  ...
  const { numTests, playwrightJobs, ver } = resolveJobsOption(childArgs.ver);
  const { imageDir, args } = childArgs;

  const jobs = Math.min(playwrightJobs, parallel);
  ...
  const execChildren = async (backends) => {
    log(
      JSON.stringify({
        ...
        playwrightNumJobs: backendDefs.playwright.jobs,
      })
    );
